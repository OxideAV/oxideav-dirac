...
| Dirac core-syntax inter **decode** | OBMC motion compensation + §11.3 residue; the two integer-pel 320×240 inter fixtures (`i-then-p`, `i-p-b`) are bit-exact. The quarter-pel interlaced LeGall-5,3 fixture is now 99.91% pixel-exact after the round-404 §15.8.5 per-component intra-DC fix (C2/V blocks used to predict from the C1/U DC value — its interior is now bit-exact); the residual ~0.09% (luma max 5, chroma max 2) is a sub-LSB picture-edge divergence, pinned by a `Tier::Bounded` corpus gate. The §15.8.10/§15.8.11 quarter-pel sub-pel primitives are also pinned against hand-computed spec values |
