...
pub const PROB_LUT: [u16; 256] = [
    ...
    1584, 1606, 1628, 1649, 1671, 1694, 1716, 1738, 1760, 1783, 1806, 1828, 1851, 1874, 1897,
    1920, 1935, 1942, 1949, 1955, 1961, 1968, 1974, 1980, 1985, 1991, 1996, 2001, 2006, 2011,
    2016, 2021, 2025, 2029, 2033, 2037, 2040, 2044, 2047, 2050, 2053, 2056, 2058, 2061, 2063,
    2065, 2066, 2068, 2069, 2070, 2071, 2072, 2072, 2072, 2072, 2072, 2072, 2071, 2070, 2069,
    2068, 2066, 2065, 2063, 2060, 2058, 2055, 2052, 2049, 2045, 2042, 2038, 2033, 2029, 2024,
    2019, 2013, 2008, 2002, 1996, 1989, 1982, 1975, 1968, 1960, 1952, 1943, 1934, 1925, 1916,
    1906, 1896, 1885, 1874, 1863, 1851, 1839, 1827, 1814, 1800, 1786, 1772, 1757, 1742, 1727,
    1710, 1694, 1676, 1659, 1640, 1622, 1602, 1582, 1561, 1540, 1518, 1495, 1471, 1447, 1422,
    1396, 1369, 1341, 1312, 1282, 1251, 1219, 1186, 1151, 1114, 1077, 1037, 995, 952, 906, 857,
...
impl ContextBank {
    ...
    pub fn update(&mut self, ctx: usize, value: bool) {
        ...
        // Clamp into (0, 0xFFFF]. The table is tuned so that neither
        // endpoint is reached in practice, but we guard against it.
        let clamped = if new_p < 1 {
            1
        } else if new_p > 0xFFFF {
            0xFFFF
        } else {
            new_p
        };
...
mod tests {
    ...
    fn prob_lut_endpoints() {
        ...
        assert!(PROB_LUT.iter().any(|&v| v == 2072));
